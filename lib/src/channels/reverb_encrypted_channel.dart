...
import 'reverb_private_channel.dart';
...
/// });
/// ```
class ReverbEncryptedChannel extends ReverbPrivateChannel {
  ...
  void _validateEncryptionKey() {
    ...
    try {
      final decoded = Base64Util.decode(encryptionMasterKey);
  ...
  dynamic _decryptEventData(dynamic data) {
    ...
    try {
      ...
      final keyBytes = Base64Util.decode(encryptionMasterKey);
      final key = encrypt.Key(keyBytes);

      // Decode the IV (nonce)
      final ivBytes = Base64Util.decode(nonce);
      ...
      return JsonUtil.decode(decrypted);
    } catch (e) {
