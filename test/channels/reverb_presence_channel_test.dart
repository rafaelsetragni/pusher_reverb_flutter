...
void main() {
  group('PresenceChannel', () {
    ...
    ReverbPresenceChannel createPresenceChannel({
      Map<String, dynamic>? channelData,
    }) {
      return ReverbPresenceChannel(
    ...
    group('subscription', () {
      test('should subscribe to a remote channel', () async {
        final channel = createPresenceChannel();
        expect(sentMessages.length, 0);

        // First subscription
        expect(
          () async => await channel.subscribe(socketId: '1234.5678'),
          returnsNormally,
        );
      ...
      test('should send channel_data during subscription', () async {
        ...
        final channel = ReverbPresenceChannel(
          name: 'presence-test-channel',
          authEndpoint: 'https://auth.example.com',
          authorizer: (_, __) async => {'Authorization': 'Bearer token'},
          channelAuthenticator: MockChannelAuthenticator(
            response: {
              'auth': 'auth-key-123',
              'channel_data': jsonEncode(testChannelData),
            },
          ),
        ...
        await channel.subscribe(socketId: 'socket-id-001');

        final message = jsonDecode(sentMessages.first) as Map<String, dynamic>;
        final data = message['data'] as Map<String, dynamic>;

        expect(data['channel'], equals('presence-test-channel'));
        expect(data['auth'], equals('auth-key-123'));
        expect(data['channel_data'], equals(jsonEncode(testChannelData)));
      });
      ...
      test('should not subscribe if already subscribed', () async {
        ...
        // Second subscription should be ignored
        expect(
          () async => await channel.subscribe(socketId: '1234.5678'),
          returnsNormally,
        );
