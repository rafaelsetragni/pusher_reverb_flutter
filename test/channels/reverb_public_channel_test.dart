  ...
  group('Channel', () {
    ...
    group('Constructor', () {
      ...
      test('throws error for empty channel name', () {
        expect(
          () => ReverbPublicChannel(name: '', sendMessage: (message) {}),
      ...
      test('throws error for channel name exceeding 200 characters', () {
        ...
        expect(
          () => ReverbPublicChannel(name: longName, sendMessage: (message) {}),
          throwsA(isA<InvalidChannelNameException>()),
        );
      });
      ...
      test('accepts valid channel names with special characters', () {
        ...
        for (final name in validNames) {
          expect(
            () => ReverbPublicChannel(name: name, sendMessage: (message) {}),
            returnsNormally,
          );
        }
      });
    ...
    group('Subscription', () {
      test('sends pusher:subscribe message when subscribing', () async {
        // Act
        await channel.subscribe(socketId: socketId);
    ...
    group('State Listeners', () {
      ...
      test('does not notify listeners when state does not change', () {
        ...
        channel.addStateListener(listener);
        channel.subscribe(socketId: socketId); // First state change
        stateChanges.clear();

        // Act
        channel.subscribe(socketId: socketId); // Should not change state
