...
void main() {
  ...
  group('ConnectionException', () {
    test('should create exception for not configured connection', () {
      // Arrange & Act
      final exception = ConnectionException.notConfigured();

      // Assert
      expect(exception.message, equals('Not connected to server'));
      expect(exception.cause, isNull);
      expect(exception.toString(), contains('ConnectionException'));
      expect(exception.toString(), contains('Not connected to server'));
  ...
  group('ChannelException', () {
    test('should create exception with message only', () {
      // Arrange & Act
      final exception = ChannelException.withoutAuthentication();

      // Assert
      expect(
        exception.message,
        equals('Private channels requires authentication'),
      );
      expect(exception.channelName, isNull);
      expect(
        exception.toString(),
        equals('ChannelException: Private channels requires authentication'),
      );
      expect(exception, isA<PusherException>());
      expect(exception, isA<Exception>());
    });
    ...
    test('should create exception with message and channel name', () {
      // Arrange & Act
      final exception = ChannelException.mismatchedChannelType(
        'private',
        'private-chat',
      );
      ...
      expect(exception.toString(), contains('channel'));
      expect(exception, isA<PusherException>());
      expect(exception, isA<Exception>());
    });
    ...
    test('toString should format channel name correctly', () {
      ...
      expect(
        exception.toString(),
        equals(
          'ChannelException: Channel already exists as a different channel type. Cannot convert to presence channel',
        ),
      );
    });

    test('should create exception for missing auth key', () {
      // Arrange & Act
      final exception = ChannelException.missingAuthKey();

      // Assert
      expect(
        exception.message,
        equals('Private channels require authentication'),
      );
      expect(exception.toString(), contains('ChannelException'));
      expect(
        exception.toString(),
        contains('Private channels require authentication'),
      );
      expect(exception, isA<PusherException>());
      expect(exception, isA<Exception>());
    });
  ...
  group('Exception Hierarchy', () {
    test('should have correct exception hierarchy', () {
      ...
      final channelEx = ChannelException.withoutAuthentication();
    ...
    test('should allow catching all exceptions with PusherException', () {
      ...
      final exceptions = <PusherException>[
        ...
        ChannelException.withoutAuthentication(),
